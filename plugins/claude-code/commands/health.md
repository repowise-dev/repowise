---
description: Show Repowise code-health — KPIs, lowest-scoring files, refactoring targets, trends, or per-file biomarkers.
allowed-tools: Bash, Read
---

# Repowise Health

Report the code-health layer: a deterministic 1–10 score per file from biomarkers
...
2. Decide the mode from `$ARGUMENTS` (see below), run the command, and present a
   readable summary — score(s), the top biomarker findings, and what they mean.
...
- a coverage file (e.g. `cov.lcov`, `coverage.xml`) → `repowise health --coverage <file>` to light up untested-hotspot / coverage-gap biomarkers (LCOV, Cobertura, Clover)
