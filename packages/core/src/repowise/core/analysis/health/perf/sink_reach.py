...
same :class:`~.callgraph.CallGraphIndex`, the same bare-sink set, the same
multi-source reverse BFS, and the same per-function first-hop name match. A fix
to the walk had to be made in both places or it silently applied to one marker
only, which is what this module exists to prevent.

The soundness limits are the ones documented on :mod:`.crossfn` -- static call
graph, depth bound, first-hop name match, and incremental runs -- since this is
that walk, not a new one.
"""
