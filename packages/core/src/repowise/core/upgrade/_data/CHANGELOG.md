...
A decision record is a claim about why the code is shaped the way it is, and almost none of the claims this layer made could be backed. On this repository's own store — 625 records, zero acceptances — `get_why` stamped `answer_basis: "decision"` on the mere presence of a record, so most calls reported a ruling under titles nobody had agreed to. A record mined from a 42-file commit claimed governance over all 42 files. Confidence ranked records filling one body field above records filling five, with 65% of ordered pairs running backwards. A hand-written decision was invisible to every path-scoped surface, because `decision add` wrote a file list and never the graph links that deliver it. The session lane had captured 139 records, 115 of them carrying neither context nor rationale, and twenty read by hand turned out to be working agreements from a transcript — "pause before pushing", "avoid duplication" — rather than anything the codebase had decided.

This release makes the layer say only what it can support. Acceptance requires a stated reason and records who, or what, signed it. Scope is bound to code the repository actually has. A working agreement is a separate kind from a decision about the code, because only the second can be checked against a diff. Session mining is off unless asked for. And for the first time the layer asks: after a commit, an agent can be offered the `decision add` line for the choice it just made, behind five gates and off by default — the agent knows what it chose and what it rejected, where a miner reading the message afterwards can only infer.

Next to it, a fourth deterministic analysis joins the pipeline. Documentation makes assertions about code, and this repository has the graph to check them: the drift pass reads the repository's own markdown, extracts the claims each document makes, resolves them against the real tree, and reports the ones the tree refutes. Savings accounting stops fabricating rates and prices each event from its own evidence. Test quality becomes something health can measure — a test that checks nothing, a test that is mostly mock setup — with a per-language assertion vocabulary underneath it, since the old rule counted 87 assertions across a 502-test-file Go repository. And a long precision pass lands on Rust, C#, Go, Objective-C, Swift and TypeScript extraction.
