"""Shared C#/VB.NET import-resolution core.

Resolution algorithm (in priority order):

1. Build (and cache) a ``DotNetProjectIndex`` for the repo. This parses
   every ``.csproj``/``.vbproj`` and ``.sln``, builds a namespace → file
   map, and collects implicit + global usings per project.
...
2. Locate the project enclosing the importer file. If unknown, fall back
   ...
   of source files repos that have no project file at all.

3. Look up the ``using``/``Imports`` namespace in the namespace map:
       ...
       c. Otherwise pick the first match anywhere in the repo (rare —
          a source file outside any project).

4. If no match is found and the namespace prefix matches a declared
   ``<PackageReference>``, register an external NuGet node.

5. Final fallback: register a generic external node so the import is
   visible in the graph even if unresolvable.

The legacy stem-match path is preserved so repos without a project file
keep working. ``ext`` (``.cs`` or ``.vb``) only affects that legacy path —
everything else operates purely on the shared, language-neutral index.
"""
...
def _legacy_stem_resolve(module_path: str, ctx: ResolverContext, ext: str) -> str | None:
    """Original 26-line resolver — used when no project index is available."""
    parts = module_path.split(".")
    local = parts[-1]
    result = ctx.stem_lookup(local.lower())
    if result and result.endswith(ext):
        return result
    if len(parts) > 1:
        dir_suffix = "/".join(parts)
        for p in ctx.sorted_paths:
            if p.endswith(ext) and dir_suffix.lower() in p.lower():
                return p
    return None
...
def _repo_root_resolved(index: object, repo_path: Path) -> Path:
    ...
    A .NET monorepo can fire tens of thousands of resolve calls; resolving
    the repo root each time accounts for a measurable chunk of
    ``graph.imports`` wall-clock on Windows.
    """
...
def _resolve_importer(index: object, repo_path: Path, importer_path: str) -> Path:
    ...
    Each source file has multiple import directives and each one triggers
...
def resolve_dotnet_import(
    module_path: str, importer_path: str, ctx: ResolverContext, ext: str
) -> str | None:
    """Resolve a using/Imports directive to a repo-relative file path or external key.

    *ext* (``.cs`` or ``.vb``) is only consulted by the legacy stem-match
    fallback — the primary namespace-map lookup is shared and
    language-neutral (see module docstring).
    """
    index = get_or_build_index(ctx)
    if index is None or not ctx.repo_path:
        # No repo path — fall back to stem-match only.
        legacy = _legacy_stem_resolve(module_path, ctx, ext)
    ...
    # component. The index resolved every source file once at build time,
    # so we cache the importer's resolved path on the index keyed by the
    ...
    # Last resort: legacy stem-match (catches repos with no project file).
    legacy = _legacy_stem_resolve(module_path, ctx, ext)
    if legacy:
        return legacy
