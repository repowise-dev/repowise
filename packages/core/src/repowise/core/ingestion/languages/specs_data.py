"""Language specifications — the declarative data behind ``LanguageRegistry``.

This is pure data: one :class:`LanguageSpec` instance per supported language
(the dataclass itself lives in ``spec.py``). It was extracted verbatim from
``registry.py`` so the registry module is just the lookup interface over this
table. Import the singleton ``REGISTRY`` from ``registry`` rather than reaching
into ``_SPECS`` directly.
"""
