...
class _CSharpExtensionHandler:
    """Host-builder extension-method scan — runs on ANY C# repo (desktop .NET too)."""

    def detect(self, dctx: DetectionContext) -> bool:
        return _has_csharp_files(dctx.parsed_files)
    ...
    def add_edges(
        ...
        return _add_csharp_extension_edges(graph, parsed_files, path_set)


HANDLERS: list[FrameworkHandler] = [_AspNetHandler(), _CSharpExtensionHandler()]
