...
def _warmup_godot(ctx: ResolverContext) -> None:
    ...
    **Its known failure mode**, and it is not hypothetical: a publisher that
    ...
    PLAN.md's leaning — vendored unless the project declares no plugin entry
    for it — reaches the same verdict on both corpus repos. This rule differs
    only for a vendored plugin that is switched *off*, which
    ``[editor_plugins] enabled=`` would not list and which this treats as
    vendored anyway.
    """
    graph = getattr(ctx, "graph", None)
    if graph is None:
...
_WARMUPS: dict[str, tuple[str, Warmup]] = {
    ...
    # first-party .gd at all. Shared phase name, and the warmup is a no-op
