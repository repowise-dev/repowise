...
class EmbedderConfigError(Exception):
    """Raised when an embedder configuration is malformed."""
    pass


def parse_numeric_env(raw_value: str, env_name: str, is_int: bool = False) -> int | float:
    """Parse a numeric environment variable.
    
    Raises EmbedderConfigError if the value is not a valid number.
    """
    try:
        value = int(raw_value) if is_int else float(raw_value)
        if value <= 0:
            raise ValueError
        return value
    except ValueError:
        raise EmbedderConfigError(f"Invalid {env_name}: {raw_value!r}")
...
def resolve_embedding_timeout(
    ...
    for name in names:
        ...
        if not raw:
            continue
        value = parse_numeric_env(raw, name)
