...
def _lookup_cost(model_name: str) -> tuple[float, float]:
    ...
    # `codex_cli/`, `claude_cli/`, `opencode/`, and `omp/` are genuinely free,
    # so they keep their prefixes.
    if "/" in lower and not lower.startswith(("codex_cli/", "claude_cli/", "opencode/", "omp/")):
