...
page, the cycle (SCC) pages it belongs to, and the repo-wide overview and
onboarding pages. Backlinks on other
...
* ``dependents`` — also regenerate the module and SCC pages that contain a
  seeded file; mark the repo-wide pages stale.
* ``full`` — also regenerate the repo-wide pages. Nothing is left stale.

The dependent set is computed structurally from the selection graph, so it is
exact rather than a heuristic cascade.
"""
...
class PageDependencies:
    ...
    Built once per run from the in-memory selection. All values are page ids
    (``module_page:...``, ``scc_page:...``), ready to drop straight into an
    ``only_page_ids`` set.

    Layers used to appear here as a third container. They no longer have pages
    of their own, so a file rolls up into nothing at that altitude and there is
    nothing to regenerate when it changes.
    """
...
def expand_cascade(
    ...
    The container set (module/SCC pages of the seeded files) and the
