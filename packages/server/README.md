...
| **MCP Server** | 19 registered MCP tools (10 advertised by default in single-repo mode: the canonical set) for AI coding assistants (Claude Code, Cursor, Cline) |
...
repowise registers 19 MCP tools and advertises **10 by default** in single-repo
mode (10 advertised by default, exactly the canonical set). Workspace mode adds
`list_repos`; eight specialist tools are opt-in where eligible. See
