...
/** What the commit is and the risk signals the analyzer scored it on. */
function commitFacts(c: CommitPromptInput, short: string): string {
  return bulletList([
...
export function buildCommitAiPrompt({
  ...
  return joinSections([
    ...
    "",
    `## Commit to review${repoSuffix(repoName)}`,
    "",
    commitFacts(c, short),
    "",
