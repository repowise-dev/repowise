...
/** The dependency-graph verdict, spelled out, keyed by `structural`. */
const STRUCTURAL_VERDICT = new Map<string, string>([
  [
    "unexplained",
    "Dependency graph: **nothing connects them** — no import, type use, framework wiring, or read. This is the finding: they move together with no structural reason to.",
  ],
  [
    "corroborated",
    "Dependency graph: a dependency already connects them, so the co-change is at least partly explained. Judge whether the dependency is the *right* one before treating this as accidental.",
  ],
  [
    "not_applicable",
    "Dependency graph: at least one side was never parsed (a lockfile, changelog, config, or doc), so there was no edge to look for. The coupling is real but is probably release plumbing, not a code-structure problem.",
  ],
]);
...
function pctOf(v: number | null | undefined): string | null {
...
function directionalConfidence(edge: CouplingPromptEdge): string | null {
  const abPct = pctOf(edge.confidence_ab);
  const baPct = pctOf(edge.confidence_ba);
  if (abPct && baPct) {
    return `Directional confidence: ${abPct} of A's own commits also touched B; ${baPct} of B's also touched A. The larger share is the stronger claim; the smaller one says how independent that side still is.`;
  }
  if (abPct) {
    return `Directional confidence: ${abPct} of A's own commits also touched B (the other side's commit total is unknown).`;
  }
  if (baPct) {
    return `Directional confidence: ${baPct} of B's own commits also touched A (the other side's commit total is unknown).`;
  }
  return null;
}
...
function couplingFacts(edge: CouplingPromptEdge, nodes: BuildCouplingPromptOptions["nodes"]): string {
  ...
  return bulletList([
    ...
    edge.support
      ...
      : null,
    directionalConfidence(edge),
    STRUCTURAL_VERDICT.get(edge.structural ?? "") ?? null,
    edge.strength != null
...
export function buildCouplingAiPrompt({
  ...
  return joinSections([
    ...
    "",
    `## Hidden coupling to untangle${repoSuffix(repoName)}`,
    "",
    couplingFacts(edge, nodes),
    "",
