...
type FileGroup = [path: string, findings: DeadCodePromptFinding[]];

function reclaimableLines(findings: DeadCodePromptFinding[]): number {
  return findings.reduce((s, f) => s + (f.lines ?? 0), 0);
}

/** Findings grouped per file, the file with the most reclaimable lines first. */
function groupByFile(findings: DeadCodePromptFinding[]): FileGroup[] {
  const byFile = new Map<string, DeadCodePromptFinding[]>();
  for (const f of findings) {
    byFile.set(f.file_path, [...(byFile.get(f.file_path) ?? []), f]);
  }
  return Array.from(byFile.entries()).sort(
    (a, b) => reclaimableLines(b[1]) - reclaimableLines(a[1]),
  );
}

function fileEntry([path, fs]: FileGroup): string {
  ...
  const risk = Array.from(new Set(fs.flatMap((f) => f.risk_factors ?? [])));
  return [
...
export function buildDeadCodeAiPrompt({
  ...
  const hidden = files.length - MAX_DEAD_CODE_FILES;

  return joinSections([
    FLAVOR_PREAMBLE[flavor],
    "",
    `## Dead-code cleanup${repoSuffix(repoName)}`,
    "",
    bulletList([
      `Files in this pile: **${files.length}**`,
      `Estimated reclaimable lines: **${reclaimableLines(findings).toLocaleString()}**`,
      "Source: repowise dead-code analysis (high-confidence, safe-to-delete tier).",
    ]),
    "",
    "## Files to clean up (largest first)",
    "",
    files.slice(0, MAX_DEAD_CODE_FILES).map(fileEntry).join("\n"),
    hidden > 0
      ? `…and ${hidden} more file${pluralS(hidden)} in the same pile (open the dead-code report in repowise for the full list).`
      : "",
    "",
