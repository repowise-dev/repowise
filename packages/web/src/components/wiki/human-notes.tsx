...
/**
 * Human-curated note pinned above a page's generated content, now editable
 * in place. Notes survive regeneration server-side.
 */
export function HumanNotes({
  ...
  async function save() {
    setSaving(true);
    try {
      const updated = await updatePageNotes(pageId, draft.trim() || null);
      setNotes(updated.human_notes);
      setDraft(updated.human_notes ?? "");
      setEditing(false);
