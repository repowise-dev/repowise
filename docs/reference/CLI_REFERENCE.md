...
| `--project-files` / `--no-project-files` | Generate or skip editor files outside `.repowise`: root `.mcp.json`, `.claude/CLAUDE.md`, `AGENTS.md`, `.vscode/`, and `.codex/`. An explicit choice is persisted under `editor_files` so later `update` runs preserve it; omit the flag to use the saved per-file preferences. Individual file/integration flags such as `--agents` and `--codex` take precedence when both are supplied. |
