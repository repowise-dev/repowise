...
class TestBindingPass:
    ...
    def test_a_binding_in_the_consumer_links_both_rows(self) -> None:
        contracts = [
            _publisher(),
        ...
        links = match_contracts(contracts)
        consumer_ids = sorted(lk.consumer_contract_id or lk.contract_id for lk in links)
        assert consumer_ids == ["topic::audit", "topic::orders"]
    ...
    def test_the_exact_pass_honours_the_routing_key_too(self) -> None:
