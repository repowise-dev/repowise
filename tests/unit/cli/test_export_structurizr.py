...
def test_a_hand_written_workspace_is_never_overwritten(tmp_path: Path, patched, capsys) -> None:
    """--standalone targets workspace.dsl, which is what a Structurizr user's
    own hand-written workspace is already called. Clobbering it would destroy
    views, styles and docs that live nowhere else."""
    mine = tmp_path / STANDALONE_FILENAME
    mine.write_text('workspace "mine" {\n  # my views\n}\n', encoding="utf-8")
    ...
    assert code == 1
    assert mine.read_text(encoding="utf-8").startswith('workspace "mine"')
    out = capsys.readouterr().out
    # Both ways out are named: include the fragment, or overwrite deliberately.
    assert "--force" in out
    assert FRAGMENT_FILENAME in out


def test_force_overwrites_a_file_that_is_not_ours(tmp_path: Path, patched) -> None:
    mine = tmp_path / STANDALONE_FILENAME
    mine.write_text('workspace "mine" {}\n', encoding="utf-8")
    ...
    assert code == 0
    assert "generated by Repowise" in mine.read_text(encoding="utf-8")


def test_our_own_output_is_replaced_without_asking(tmp_path: Path, patched) -> None:
    """Re-exporting is the normal workflow; a prompt every time would be noise."""
    for _ in range(2):
        code = export_structurizr(
            tmp_path,
            output=None,
        ...
        assert code == 0
    assert (tmp_path / FRAGMENT_FILENAME).exists()


def test_a_binary_file_at_the_destination_is_treated_as_not_ours(tmp_path: Path, patched) -> None:
    """An undecodable file must fail closed — it is certainly not our text."""
    target = tmp_path / "arch.dsl"
...
def test_the_standalone_message_leads_with_the_zero_install_route(
    tmp_path: Path, patched, capsys
) -> None:
    """A docker command is not an answer for someone without docker."""
    export_structurizr(
        tmp_path,
        output=None,
        standalone=True,
    ...
    out = capsys.readouterr().out
    assert "structurizr.com" in out
    assert out.index("structurizr.com") < out.index("docker run")
