...
|------|------|---------|-------------|
| `--provider` | string | auto | LLM provider: `anthropic`, `openai`, `openrouter`, `gemini`, `deepseek`, `kimi`, `minimax`, `ollama`, `litellm`, `codex_cli`, `opencode`, `mock` |
...
3. API key environment variables: `ANTHROPIC_API_KEY` -> `OPENAI_API_KEY` -> `OPENROUTER_API_KEY` -> `OLLAMA_BASE_URL` -> `GEMINI_API_KEY` -> `DEEPSEEK_API_KEY` -> `KIMI_API_KEY` -> `MINIMAX_API_KEY`
