...
The MCP server sits on top of the persistence layer and exposes everything to AI coding assistants through 19 registered tools: 11 by default in a single repository, 13 by default in workspace mode, and five opt-in tools. It's the primary interface between repowise and Claude Code, Codex, Cursor, Cline, or any other MCP-compatible editor.
